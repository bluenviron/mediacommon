...
func (e *AudioMuxElement) Unmarshal(buf []byte) error {
	...
	e.Payloads = make([]PayloadMux, e.StreamMuxConfig.NumSubFrames+1)
	...
	for i := range e.StreamMuxConfig.NumSubFrames + 1 {
		...
		err = e.Payloads[i].unmarshalBits(buf, &pos, &li)
		if err != nil {
			return err
		}
	}
	...
	for i := range e.StreamMuxConfig.NumSubFrames + 1 {
		...
		for prog, p := range e.StreamMuxConfig.Programs {
			...
			for lay := range p.Layers {
				err := li.setPayloadLength(prog, lay, uint64(len(e.Payloads[i].Payloads[prog][lay])))
				if err != nil {
					return 0
				}
			}
		...
		n += e.Payloads[i].marshalSizeBits()
	}
...
func (e AudioMuxElement) Marshal() ([]byte, error) {
	...
	for i := range e.StreamMuxConfig.NumSubFrames + 1 {
		...
		e.Payloads[i].marshalToBits(buf, &pos)
	}
