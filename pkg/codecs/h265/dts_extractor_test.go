...
var casesDTSExtractor = []struct {
	...
	{
		"issue mediamtx/6031 (temporal sub-layers)",
		[]sequenceSample{
			{
				[][]byte{
					{ // VPS
...
// TSA and STSA pictures have the same slice segment header syntax as TRAIL
// pictures (Rec. ITU-T H.265, table 7-1) and must be processed in the same way.
func TestDTSExtractorTemporalSubLayers(t *testing.T) {
	for _, mapping := range []struct {
		name         string
		nonReference NALUType
		reference    NALUType
	}{
		{"TSA", NALUType_TSA_N, NALUType_TSA_R},
		{"STSA", NALUType_STSA_N, NALUType_STSA_R},
	} {
		for _, ca := range casesDTSExtractor {
			t.Run(mapping.name+", "+ca.name, func(t *testing.T) {
				var ex DTSExtractor
				ex.Initialize()

				for _, sample := range ca.sequence {
					au := make([][]byte, len(sample.au))

					for i, nalu := range sample.au {
						var typ NALUType

						switch NALUType((nalu[0] >> 1) & 0b111111) {
						case NALUType_TRAIL_N:
							typ = mapping.nonReference

						case NALUType_TRAIL_R:
							typ = mapping.reference
						...
						au[i] = append([]byte(nil), nalu...)
						au[i][0] = (nalu[0] & 0b1) | (byte(typ) << 1)
					}

					dts, err := ex.Extract(au, sample.pts)
					require.NoError(t, err)
					require.Equal(t, sample.dts, dts)
				}
			})
